...
// А вот если типы, которыми мы параметризованы не совпадают, то это уже ошибка компиляции, опять же по причине не совместимы по присваиванию
```
...
List<Test> tl; Test t = max(tl); // CE, Generics несовместимы по присваиванию, поэтому баним всё то, что не <T extends Comparable<T>>
...
public boolean equals(Object o) {
    if (o instanceof Point) {
        Point that = (Point) o; 
        // В следующей строке важно that.canEqual() - в одну сторону мы уже проверили, что можем сравниваться в if, чтобы проверить в другую - вызываем that.canEqual
        return (that.canEqual(this)) && this.x == that.x && this.y == that.y && this.c == that.c;
    }
    return false;
}
